...
import { BRANDS, ALLOWED_BRANDS, brandWrap } from "../../lib/brand-doc";
import { revalidatePath } from "next/cache";

// Brand identity + the branded printable shell now live in the ONE shared module
// lib/brand-doc.ts so the Studio, grant-ready docs, reports, and invoices all
// render through the same letterhead/print CSS/PDF path (P8). No local fork here.
...
// The branded printable shell + escapeHtml now live in lib/brand-doc.ts (imported
// above) so every document type renders through the one letterhead/print path.
