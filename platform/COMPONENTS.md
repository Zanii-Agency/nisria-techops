...
Last reconciled: R4-A (2026-05-26).

---

## R4-A — "text gets cut" class + outdated grants (one pass)

The rule (the founder's): nothing truncates, everything scrolls or sizes to its
content, nothing overlaps. Fixed across the board, not per-symptom:

- **FocusTab / message bodies never sliced.** The "In reply to" original is no
  longer `.slice(0, 900)` in `app/page.tsx`; it reads the FULL original from the
  `messages` row by `message_id` and scrolls inside `.peek-quote` (now `46vh`
  tall, then its own scroll). The stored `context.original` cap in
  `app/api/agents/tick/route.ts` was raised 1200 → 8000 (the display reads the
  full message row anyway; this is only a fallback). The inbox thread body is no
  longer `.slice(0, 2500)`; `.mail-read` scrolls the full message. `.sheet-body`
  remains the one scroll owner of the tab.
- **Letterhead/brand `<select>` fits its label.** `StudioConsole.tsx` (the
  "Nisria letterhead" select that clipped to "Nisria letterhea⌄") and the
  Report/Invoice builder letterhead selects use `width:auto` + a `min-width`
  instead of a fixed `max-width`, so the longest option is never cut.
- **No card overlap.** Every `.card` now has `position: relative; isolation:
  isolate` (a clean self-contained stacking context), and `.studio-intake` also
  `isolation: isolate`, so a card / its hover transform / an inner absolutely-
  positioned hint can never bleed over the section below it (R4-2 / Studio
  "Recent documents").

**Outdated grants (R4-3).** `app/grants/page.tsx` filters expired items out of
the actionable surfaces. `isExpired(deadline, now().today)` drops opportunities
whose `close_date` (free text, parsed MM/DD/YYYY or ISO) is strictly before
today, and drops grant applications with a past `deadline` from the
Researching / Prepared / Submitted columns (Won/Lost history may keep an old
deadline; an unknown/null deadline is never treated as expired). Uses the ONE
clock (`lib/now.ts`), so "today" is the tz-aware server date, never hard-coded.
...
**No tool/AI-authorship footer (R4-A / R4-7):** the default footer is now just
the org's legitimate letterhead line (`BRANDS[key].legal`, e.g. "By Nisria Inc ·
501(c) nonprofit · EIN 92-2509133"). The old "Prepared with the Nisria Command
Center" / "Generated by the Nisria Command Center" watermark was removed (it
revealed how the document was made; nothing outside should know). `report-builder.ts`
no longer passes a `footNote`, so it inherits the clean default; the invoice
footer is the legal line + invoice number. Existing studio HTML was re-cleaned
of the old footer string by `scripts/reclean-r4a.mjs`.
...
**Humanize-on-RENDER (R4-A / R-recur-1):** humanize is now also applied at the
DISPLAY layer, not only at generation, so stored content made BEFORE the gate
(old grant packages, old studio docs) renders clean regardless of when it was
created. The fix is two-part and both halves shipped:
- **render:** `components/GrantPeek.tsx` runs the prepared package body through
  `humanize()` (via `renderClean`) right before `renderMarkdown`, after
  resolving the `⟦GRANT_DATE⟧` token, so a pre-gate row with "— —" shows clean.
- **data re-clean:** `platform/scripts/reclean-r4a.mjs` (one-time, run via the
  Supabase Management API) stripped em-dashes + bracket placeholders from every
  existing `grant_applications.notes` row (7 rows) and em-dashes + the old
  tool-authorship footer from `studio_documents.html` (1 row), preserving the
  live-date token. Principle: any data-class fix RE-CLEANS existing rows, never
  only new ones. Studio docs render from stored HTML in a sandboxed iframe, so
  they cannot humanize-on-render; the re-clean cleans the stored bytes instead.
...
- `lib/agents/grant.ts` (`buildApplication`) — package body humanized; the cover
  ...
  Displayed package also re-humanized on render (above).
...
`open-cmdk` window event. Renders CRISP and truly CENTERED: a plain dim scrim
(no backdrop blur) and a fully OPAQUE solid panel at `--z-cmdk` (above the dock,
below modals). The top-nav search button dispatches `open-cmdk`.

**R-recur-2 root fix (R4-A):** it previously used cmdk's `Command.Dialog`, whose
...
rendered bottom-left, uncentered (the recurring bug R3-1 only re-colored). It
now renders its OWN overlay with the panel as a CHILD, using the EXACT structure
of the Modal / FocusTab primitives (`.cmdk-overlay { position:fixed; inset:0;
display:grid; place-items:center }` + the `.cmdk-panel` child), so centering is
guaranteed and no longer depends on a portal we do not control. The `cmdk`
`Command` engine (filter + keyboard list) is kept; only the broken `Dialog`
wrapper was dropped. The palette also locks body scroll + focuses the input and
closes on Esc / scrim-click, mirroring the Modal.
