...
export const SMART_TOOLS = [
  ...
  { name: "edit_event", description: "EDIT an existing calendar event IN PLACE: change its title, date, time, location, or notes. Use for 'rename the DDW meeting to X', 'change the meeting title', 'add a location to the donor visit', 'update the notes on the Kibera trip', 'change the meeting to 3pm and rename it'. NEVER delete-and-recreate an event to change it — this edits it directly (on the calendar and on Google). Match the event by a fragment of its CURRENT title. Provide only the fields that change. For a pure reschedule you can also use move_event; this handles everything including the title.", input_schema: { type: "object", properties: { title: { type: "string", description: "a fragment of the event's CURRENT title to find it" }, new_title: { type: "string", description: "the new title, if renaming" }, new_date: { type: "string", description: "YYYY-MM-DD, if the date changes" }, new_time: { type: "string", description: "HH:MM 24h, if the time changes. Pass it EXACTLY as said; if another timezone is named, set source_timezone and do NOT convert it yourself." }, all_day: { type: "boolean", description: "true to make it an all-day event (clears the time)" }, new_location: { type: "string", description: "the new location; empty string clears it" }, new_notes: { type: "string", description: "the new notes/description; empty string clears them" }, source_timezone: { type: "string", description: "ONLY if new_time is stated in a zone other than Dubai (e.g. 'Nairobi', 'EAT', 'UTC'); the system converts it." } }, required: ["title"] } },
  ...
  { name: "update_grant_status", description: "EDIT a grant application IN PLACE: move its status, record an award, OR change its program name, requested amount, deadline, or notes. Use for 'mark the Ford grant submitted', 'we won the USAID grant for 40000', 'change the Ford grant deadline to 2026-09-01', 'update the requested amount on the X grant to 50000', 'rename the program on the Y grant'. NEVER delete-and-recreate a grant to change a detail. Match by funder. Provide only the fields that change.", input_schema: { type: "object", properties: { funder: { type: "string", description: "the funder, to find the grant" }, status: { type: "string", enum: ["researching", "drafting", "review", "submitted", "won", "lost", "rejected"] }, amount_awarded: { type: "number", description: "the awarded amount (USD), if won" }, amount_requested: { type: "number", description: "change the requested amount (USD)" }, deadline: { type: "string", description: "change the deadline, YYYY-MM-DD" }, new_program: { type: "string", description: "rename the grant's program" }, new_notes: { type: "string", description: "change the notes; empty string clears them" } }, required: ["funder"] } },
...
async function runAction(db: any, name: string, input: any, ctx: { sourceGroup?: string; senderPhone?: string; proofPath?: string; confirmWrites?: boolean; contactId?: string; sourceMessageId?: string; tier?: "admin" | "team"; teamCap?: "field" | "coordinator"; rank?: "owner" | "founder" | "member" | null; operatorName?: string; casesIntake?: boolean; forceResend?: boolean; userText?: string; viaBridge?: boolean; traceId?: string } = {}): Promise<ToolResult> {
  ...
  // ---- SAFE: save_vault_resource (Resources hub / vault) ----
  if (name === "save_vault_resource") {
    const title = String(input.title || "").trim();
  ...
  if (name === "update_grant_status") {
    ...
    // FULL EDIT (2026-07-22): a grant's program, requested amount, deadline, or notes can be
    // changed IN PLACE too — no delete-and-recreate to fix a deadline or rename a program.
    if (typeof input.new_program === "string" && input.new_program.trim()) { patch.program = input.new_program.trim().slice(0, 200); changed.push(`program "${patch.program}"`); }
    if (typeof input.amount_requested === "number" && input.amount_requested >= 0) { patch.amount_requested = input.amount_requested; changed.push(`requested ${money(input.amount_requested)} USD`); }
    if (/^\d{4}-\d{2}-\d{2}$/.test(String(input.deadline || ""))) { patch.deadline = String(input.deadline); changed.push(`deadline ${patch.deadline}`); }
    if (typeof input.new_notes === "string") { patch.notes = input.new_notes.trim() || null; changed.push(patch.notes ? "notes updated" : "notes cleared"); }
    if (!changed.length) return { ok: false, summary: humanize("Tell me what to change on the grant (status, award, requested amount, deadline, program, or notes).", opts), error: "no changes" };
  ...
  // ---- PERPETUAL PEOPLE-MEMORY: remember_person (2026-07-22) ----
  // Nur works with people she never formally saves (e.g. "Jensen"), and add_contact REQUIRES a
  // phone or email, so a name-only person could not be stored and the bot re-asked "who is X"
  // every time. This saves a LIGHTWEIGHT, name-only contact (no phone needed) so lookup_contact
  // finds them next time, and — if a relationship is given — writes it to the Brain so grounding
  // knows WHO they are. Idempotent: never duplicates someone already on file.
  if (name === "remember_person") {
    const pname = String(input.name || "").trim().slice(0, 120);
    if (!pname || pname.length < 2) return { ok: false, summary: humanize("Tell me the person's name to remember.", opts), error: "no name" };
    const rel = String(input.relationship || input.note || "").trim().slice(0, 200);
    const like = `%${pname.replace(/[(),:*%_]/g, "")}%`;
    // Already on file anywhere? Never duplicate — just refresh who-they-are if given.
    const [c, t, b] = await Promise.all([
      db.from("contacts").select("id,name").ilike("name", like).limit(1),
      db.from("team_members").select("id,name").ilike("name", like).limit(1),
      db.from("beneficiaries").select("id,full_name").ilike("full_name", like).limit(1),
    ]);
    const existing = ((c as any).data?.[0]) || ((t as any).data?.[0]) || ((b as any).data?.[0]);
    // Store who-they-are in the Brain so it surfaces in grounding next time this name comes up.
    if (rel) { try { await remember({ kind: "person", title: pname, content: `${pname} — ${rel}`, metadata: { relationship: rel } } as any); } catch { /* best-effort */ } }
    if (existing) return { ok: true, summary: humanize(`I've already got ${pname} on file${rel ? `, noted: ${rel}` : ""}.`, opts), detail: { already: true, name: pname } };
    // Name-only lightweight contact so it's findable by lookup_contact from now on.
    const { data: ins, error: rpErr } = await db.from("contacts").insert({ name: pname, channel: "mentioned" }).select("id").single();
    if (rpErr) return { ok: false, summary: humanize(`I couldn't save ${pname} just now. Want me to try again?`, opts), error: (rpErr as any).message || "save failed" };
    await emit({ type: "contact.remembered", source: "agent:sasa", actor: ctx.operatorName || "Nur", subject_type: "contact", subject_id: (ins as any)?.id || null, payload: { name: pname, relationship: rel || null, via: "remember_person" } });
    return { ok: true, summary: humanize(`Got it, I'll remember ${pname}${rel ? ` (${rel})` : ""} from now on.`, opts), detail: { name: pname, contact_id: (ins as any)?.id } };
  }
  ...
  if (name === "create_event") {
    ...
    let eventDate = date;
    // DETERMINISTIC TZ CONVERSION (2026-06-22 Nairobi/Dubai bug, KT #206540): when the user
    // states a time in ANOTHER zone ("12pm Nairobi time"), the model passes the time AS
    // STATED plus source_timezone, and CODE converts it to the operator's zone (Asia/Dubai),
    // never the model's arithmetic — it added 2h instead of 1. Converts date too (a late
    // event can roll to the next day). Fail-safe: a bad/unknown zone leaves the time as-is.
    if (String(input.source_timezone || "").trim() && time) {
      const conv = convertWallClock(eventDate, time, String(input.source_timezone), DEFAULT_TZ);
      eventDate = conv.date; time = conv.time;
      if (endTime) endTime = convertWallClock(date, endTime, String(input.source_timezone), DEFAULT_TZ).time;
    }
    // dedup: if an event with the same title and date already exists, do not duplicate
    const { data: existingEvent } = await db.from("calendar_events").select("id").eq("title", title).eq("starts_on", eventDate).limit(1);
  ...
  if (name === "invite_to_event") {
    ...
    if (!gcalConfigured()) return { ok: false, summary: humanize("I can't reach Google Calendar right now, so I couldn't send the invite. Try again shortly.", opts), error: "gcal not configured" };
    // Match meetings by title, SYNCED or app-only. Nisria's events are app-first and mostly
    // NOT on Google (gcal_event_id null), so scoping to synced-only made this inert on real
    // data (2026-07-21). App-only meetings are CREATED on Google now WITH the guest, so Google
    // emails the invite, then the id is back-written — synced-going-forward and idempotent (a
    // repeat call sees the id and merges instead of creating a duplicate).
    const { data: evs } = await db.from("calendar_events").select("id,title,starts_on,start_time,ends_on,end_time,all_day,location,notes,gcal_event_id").ilike("title", `%${frag}%`).order("starts_on", { ascending: true }).limit(10);
    const list = (evs || []) as any[];
    if (!list.length) return { ok: false, summary: humanize(`I couldn't find a calendar meeting matching "${frag}". Try a few words from the exact title.`, opts), detail: { matched: 0 } };
    const invited: string[] = [];
    let sendErr: string | null = null;
    for (const e of list) {
      try {
        if (e.gcal_event_id) {
          await addAttendeesToEvent(e.gcal_event_id, emails);
        } else {
          // DB `time` columns come back as HH:MM:SS; gcal wants HH:MM (toResource appends :SS).
          const startHHMM = e.start_time ? String(e.start_time).slice(0, 5) : null;
          let endHHMM = e.end_time ? String(e.end_time).slice(0, 5) : null;
          if (startHHMM && !endHHMM) { const [h, m] = startHHMM.split(":").map(Number); endHHMM = `${String((h + 1) % 24).padStart(2, "0")}:${String(m).padStart(2, "0")}`; } // default 1h
          const created = await gcalCreate({ title: e.title, starts_on: e.starts_on, ends_on: e.ends_on, start_time: startHHMM, end_time: endHHMM, all_day: e.all_day, location: e.location, notes: e.notes, attendees: emails } as any);
          await db.from("calendar_events").update({ gcal_event_id: created.id }).eq("id", e.id);
        }
        invited.push(e.title);
  ...
  if (name === "move_event") {
    ...
    const e = list[0];
    const inputTime = /^\d{2}:\d{2}$/.test(String(input.new_time || "")) ? String(input.new_time) : null;
    let new_date = /^\d{4}-\d{2}-\d{2}$/.test(String(input.new_date || "")) ? String(input.new_date) : e.starts_on;
    let new_time = inputTime || e.start_time;
    // DETERMINISTIC TZ CONVERSION (same as create_event, KT #206540): a NEW time stated in
    // another zone is converted in code from source_timezone -> operator zone, never the
    // model's arithmetic. Only the explicitly-provided new time is converted (not the
    // event's existing stored time).
    if (inputTime && String(input.source_timezone || "").trim()) {
      const conv = convertWallClock(new_date, inputTime, String(input.source_timezone), DEFAULT_TZ);
      new_date = conv.date; new_time = conv.time;
    }
  ...
  // ---- SAFE: edit_event — change ANY detail of an existing event IN PLACE (2026-07-22) ----
  // Before this, move_event only changed date/time, so renaming a meeting or fixing its
  // location/notes had NO tool and Nur had to DELETE + recreate it. edit_event patches any
  // subset of {title, date, time, location, notes, all_day} on the row AND on Google Calendar.
  if (name === "edit_event") {
    const frag = String(input.title || "").trim().slice(0, 40);
    if (!frag) return { ok: false, summary: humanize("Which event should I edit? Tell me a few words from its current title.", opts), error: "no title" };
    const { data: matches } = await db.from("calendar_events").select("*").ilike("title", `%${frag}%`).order("starts_on", { ascending: true }).limit(5);
    const list = (matches || []) as any[];
    if (!list.length) return { ok: false, summary: humanize("I could not find a calendar event matching that.", opts) };
    if (list.length > 1) return { ok: false, summary: humanize(`There are ${list.length} events matching that: ${list.map((ev) => `"${ev.title}" (${ev.starts_on})`).join(", ")}. Which one?`, opts) };
    const e = list[0];
    // Build the change set from ONLY the fields the caller actually supplied, so an edit to the
    // title never silently blanks the time (and vice-versa).
    const changes: Record<string, any> = {};
    const changed: string[] = [];
    if (typeof input.new_title === "string" && input.new_title.trim()) { changes.title = input.new_title.trim().slice(0, 200); changed.push(`renamed to "${changes.title}"`); }
    if (/^\d{4}-\d{2}-\d{2}$/.test(String(input.new_date || ""))) { changes.starts_on = String(input.new_date); changed.push(`moved to ${changes.starts_on}`); }
    const inputTime = /^\d{2}:\d{2}$/.test(String(input.new_time || "")) ? String(input.new_time) : null;
    if (inputTime) { changes.start_time = inputTime; changes.all_day = false; }
    if (input.all_day === true) { changes.all_day = true; changes.start_time = null; }
    if (typeof input.new_location === "string") { changes.location = input.new_location.trim() || null; changed.push(changes.location ? `location set to "${changes.location}"` : "location cleared"); }
    if (typeof input.new_notes === "string") { changes.notes = input.new_notes.trim() || null; changed.push(changes.notes ? "notes updated" : "notes cleared"); }
    // Deterministic TZ conversion for a new time stated in another zone (same rule as move_event).
    if (inputTime && String(input.source_timezone || "").trim()) {
      const conv = convertWallClock(changes.starts_on || e.starts_on, inputTime, String(input.source_timezone), DEFAULT_TZ);
      changes.starts_on = conv.date; changes.start_time = conv.time;
    }
    if (inputTime || input.all_day === true) changed.push(changes.all_day ? "set to all-day" : `time set to ${changes.start_time}`);
    if (!Object.keys(changes).length) return { ok: false, summary: humanize(`Tell me what to change on "${e.title}" — a new title, date, time, location, or notes.`, opts), error: "no changes" };
    changes.updated_at = new Date().toISOString();
    if (changes.starts_on || changes.start_time !== undefined || changes.all_day !== undefined) changes.reminded_at = null; // re-arm the reminder if timing changed
    const { error: editErr } = await db.from("calendar_events").update(changes).eq("id", e.id);
    // VERIFIED WRITE (KT #336): never say "Updated" unless the write landed.
    if (editErr) return { ok: false, summary: humanize(`I could not update "${e.title}" just now, so it is unchanged. Want me to try again?`, opts), error: (editErr as any).message || "event update failed" };
    // Sync the SAME edit to Google (merge existing + changes so untouched fields are preserved).
    const merged = { ...e, ...changes };
    if (e.gcal_event_id && gcalConfigured()) { try { await gcalPatch(e.gcal_event_id, merged); } catch { /* best-effort: DB is source of truth */ } }
    await emit({ type: "calendar.event_updated", source: "agent:sasa", actor: ctx.operatorName || "Nur", subject_type: "calendar_event", subject_id: e.id, payload: { title: changes.title || e.title, fields: Object.keys(changes).filter((k) => k !== "updated_at" && k !== "reminded_at") } });
    return { ok: true, summary: humanize(`Updated "${changes.title || e.title}": ${changed.join(", ")}.`, opts), affordance: { kind: "open", label: "Open calendar", href: "/calendar" }, detail: { event_id: e.id, changed } };
  }
