...
function toResource(ev: { title: string; starts_on: string; ends_on?: string | null; start_time?: string | null; end_time?: string | null; all_day?: boolean; location?: string | null; notes?: string | null }) {
  ...
  // Stored start_time/end_time are naive wall-clock in the app's ONE canonical
  // zone (DEFAULT_TZ = Asia/Dubai) — the portal, the brief, notify, and period
  // all read them as Dubai. This MUST tag the Google push with that same zone,
  // or the phone and the portal silently disagree by the Dubai/Nairobi gap. It
  // was hardcoded "Africa/Nairobi" here, so every timed event landed on Nur's
  // phone 1h off from the dashboard. One constant, imported, removes the drift.
  const tz = DEFAULT_TZ;
