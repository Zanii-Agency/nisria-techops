...
import { pickFromMatches, isAllDuplicates, normalizeKey } from "./match-dedup.mjs";
...
// Returns { task, cands, wasDuplicate, frag, byId }. task is null when 0 match or genuinely
// ambiguous (>1 distinct title); the caller then surfaces cands (WITH ids) so it can re-target.
export async function findTaskToActOn(db, input) {
  ...
  if (input.id && TASK_ID_RE.test(String(input.id).trim())) {
    ...
    const t = (data || [])[0] || null;
    return { task: t, cands: t ? [t] : [], wasDuplicate: false, frag: "", byId: true };
  }
  // Scrub LIKE wildcards so a title carrying %/_ cannot over-match.
  const frag = String(input.title || "").trim().replace(/[%_]/g, "").slice(0, 60);
  if (!frag) return { task: null, cands: [], wasDuplicate: false, frag, byId: false };
  ...
  if (exact.length === 1) return { task: exact[0], cands, wasDuplicate: false, frag, byId: false };
  return { task: pickFromMatches(cands, taskKey), cands, wasDuplicate: isAllDuplicates(cands, taskKey), frag, byId: false };
}
