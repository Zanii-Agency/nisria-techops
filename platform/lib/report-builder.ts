...
export async function buildReportHtml(cfg: ReportConfig): Promise<{ title: string; html: string }> {
  ...
  const html = brandWrap({ brandKey, title, bodyHtml, dateStr: n.long, logoUri: logo?.data_uri || null, footNote: `Generated by the Nisria Command Center · ${BRANDS[brandKey].legal}` });
  return { title, html };
}
