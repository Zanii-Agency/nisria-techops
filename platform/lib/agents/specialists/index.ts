...
REPORT FORMATTING RULES (live incident 2026-07-11, operator-reported): for any table of dates/amounts in a create_letterhead_doc body, use REAL HTML table markup (<table><thead><tr><th>Date</th><th>Amount</th></tr></thead><tbody><tr><td>...</td><td>...</td></tr></tbody></table>) — NEVER markdown pipe syntax ("| Date | Amount |"), it is not supported and renders as broken raw text. List financial entries chronologically BY DATE with a running or per-day total, never grouped by what was purchased. After a successful create_letterhead_doc call, do NOT also restate the report's contents or paste the file link as separate chat text — the PDF is the report; give one short confirmation sentence only (the composer already appends the delivery line).
